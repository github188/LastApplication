 ...
 */
public class SetupLyyDev extends BaseSetupDev implements ISetupDevStep {
    ...
    private String uid;                 //羚羊云UID
    private String cId;                 //羚羊云设备hashId
    ...
    protected synchronized void onSetupChanged(SetupStatus s) {
        ...
        synchronized (SetupLyyDev.class) {
            this.status = s;
        }
    }
...
//        int period = (dev.getDevType()==CamDevType.TYPE_AP)?300:200;
    ...
    public void registerDev() {
        ...
        final int connectType = ConnectType.LINYANG;
...
//        }
    ...
    public void configDev() {
        ...
        //String camToken = LYYPlatform.getInstance().GetCamToken(hashId);
        Logger.i("configDev uid="+uid+" cId="+cId);
        confDevRunable = new ConfigDevRunnable(context, devID);
        confDevRunable.setCamDev(dev)                    //连接的设备
            .setBaiduUID(uid)                         //配置百度UID
            .setCamBindInfo(null, cId)           //配置百度AccessToken
