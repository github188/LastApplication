...
//        StatisticsApi.apiBindBaiduPush();
    ...
    //查找已经注册的摄像机设备
    private CamLive findRegistedCamDev(Business retBus, String deviceId) {
        IMimeCamBusiness business = ErmuBusiness.getMimeCamBusiness();
        CamLive camLive = business.getCamLive(deviceId);
        ...
        if(camLive != null && camLive.getDataType()== LiveDataType.MIME) {
            streamId = (camLive!=null) ? camLive.getStreamId() : "";
            BusObject.publishEvent(OnSetupDevEvent.class, camLive);
            onSetupChanged(SetupStatus.REGISTED);             //设备已注册
        } else if(bus.getCode()==BusinessCode.NO_PERMISSION
                || retBus.getCode()==BusinessCode.DEV_REGISTED
                ) {
            onSetupChanged(SetupStatus.REGISTER_NOTPERMISSION);//提示该设备不是当前用户的
            Logger.oFile(bus.toString(), "SetupDev");
        } else if(bus.getCode()!=BusinessCode.SUCCESS) {
            onSetupChanged(SetupStatus.REGISTER_FAIL);        //注册设备失败
            Logger.oFile(bus.toString(), "SetupDev");
        }
        return camLive;
    }
