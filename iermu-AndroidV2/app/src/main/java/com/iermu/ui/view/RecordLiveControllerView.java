 ...
 */
public class RecordLiveControllerView extends RelativeLayout implements View.OnClickListener, AdapterView.OnItemClickListener, HorizontalListView.OnScrollListener, View.OnTouchListener {
    ...
    ImageButton buttonSelectDateCtrl;
    ...
    @ViewInject(R.id.viewTimeLineCtrl)
    View viewTimeLineCtrl;
    @ViewInject(R.id.viewThumbnailCtrl)
    View viewThumbnailCtrl;
    @ViewInject(R.id.textViewTimeLineCtrl)
    TextView textViewTimeLineCtrl;
    @ViewInject(R.id.textViewThumbnailCtrl)
    TextView textViewThumbnailCtrl;
    @ViewInject(R.id.imageViewTimeLineDot)
    ImageView imageViewTimeLineDot;
    @ViewInject(R.id.imageViewThumbnailDot)
    ImageView imageViewThumbnailDot;
    ...
    public RecordLiveControllerView(Context context) {
        super(context);
    }

    public RecordLiveControllerView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void initView() {
        ...
        viewTimeLineCtrl.setOnClickListener(this);
        viewThumbnailCtrl.setOnClickListener(this);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.viewTimeLineCtrl:
                switchToTimeLine();
                if (listener != null) {
                    listener.onSwitchToTimeLine();
                }
                delayHideCtrl();
                break;
            case R.id.viewThumbnailCtrl:
                switchToThumbnail();
                if (listener != null) {
                    listener.onSwitchToThumbnail();
                }
                delayHideCtrl();
                break;
            default:
    ...
    public void changeToPortrait() {
        ...
        RelativeLayout.LayoutParams paramsVideoView1 = (RelativeLayout.LayoutParams) videoView.getLayoutParams();
    ...
    public void switchToTimeLine() {
        textViewTimeLineCtrl.setTextColor(0xff00acef);
        imageViewTimeLineDot.setVisibility(View.VISIBLE);
        textViewThumbnailCtrl.setTextColor(0xffffffff);
        imageViewThumbnailDot.setVisibility(View.INVISIBLE);
    }

    /**
     * 切换到缩略图
     */
    public void switchToThumbnail() {
        textViewTimeLineCtrl.setTextColor(0xffffffff);
        imageViewTimeLineDot.setVisibility(View.INVISIBLE);
        textViewThumbnailCtrl.setTextColor(0xff00acef);
        imageViewThumbnailDot.setVisibility(View.VISIBLE);
    }
    ...
    public void printScreen() {
        ...
        if (listener != null) {
            listener.onPrintScreenComplete(RecordLiveControllerView.this, smallBitMap, imageTime);
        }
        ErmuApplication.execBackground(new Runnable() {
            @Override
            public void run() {
                if (listener != null) {
                    listener.onSaveScreenImageComplete(RecordLiveControllerView.this, bitmap, imageTime);
                }
            }
        });
    }
    ...
    public void setImageCut(Bitmap bitmap) {
        ...
        new Handler().postDelayed(new Runnable() {
            public void run() {
                viewCut.setVisibility(View.GONE);
                if (listener != null) {
                    listener.onCutImageHide(RecordLiveControllerView.this);
                }
            }
        }, 4000);
    }
    ...
    public interface OnControlListener {
        ...
        // 声音开关
        void onSoundClick(RecordLiveControllerView view);

        // 退出全屏
        void onOutFullScreenClick(RecordLiveControllerView view);

        // 点击云台
        void onDeleteClick(RecordLiveControllerView view);

        // 点击播放
        void onPlayClick(RecordLiveControllerView view);

        // 点击gridView上的一个日期
        void onDateItemClick(int position, RecordLiveControllerView view);

        // 点击看直播
        void onGotoLiveClick(RecordLiveControllerView view);

        // 点击截屏
        void onPrintScreenClick(RecordLiveControllerView view);

        // 截图隐藏时调用
        void onCutImageHide(RecordLiveControllerView view);

        // 点击截图图片
        void onViewCutClick(RecordLiveControllerView view);

        // 完成截图时调用
        void onPrintScreenComplete(RecordLiveControllerView view, Bitmap bitmap, long imageTime);

        // 保存截图完成时调用
        void onSaveScreenImageComplete(RecordLiveControllerView view, Bitmap bitmap, long imageTime);

        // 切换到时间轴
        void onSwitchToTimeLine();

        // 切换到缩略图
        void onSwitchToThumbnail();
    }
}
